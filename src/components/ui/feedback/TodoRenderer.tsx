...
const TodoItemComponent: React.FC<{ todo: TodoItem; theme: NexusTheme }> = ({ todo, theme }) => {
  ...
  const getStatusColor = (status: TodoItem["status"]): string => {
    switch (status) {
      case "completed":
        return theme.todo.completed; // Gray for completed
      case "in_progress":
        return theme.todo.inProgress; // Green for in progress
      case "pending":
        return theme.todo.pending; // Light gray for pending
      default:
        return theme.todo.pending;
    }
  };
  ...
  return (
    <Box marginBottom={0}>
      <Text color={theme.todo.background}> </Text>
